...
- **Centro de Masa:** En sistemas aislados, el momento lineal total $\vec{P}_{\text{tot}} = \sum_{i=1}^N m_i \vec{v}_i$ se preserva a nivel de precisión de máquina ($< 10^{-15}$).
...
El uso de funciones de activación $C^\infty$ (`Tanh`, `SiLU`) asegura derivadas segundas mixtas no nulas sin desaparición del gradiente durante el entrenamiento:
$$
...
- **Observables en Tiempo Real:** Cálculo simultáneo a 60 FPS de la energía total $H = T + U$, error relativo $\Delta H / |H_0|$, velocidad del centro de masa $\|\vec{V}_{\text{cm}}\|$, y momento angular total $\|\vec{L}\|$.
